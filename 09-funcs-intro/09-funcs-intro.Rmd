...
subtitle: "Lecture 9: Functions in R: (1) Introductory concepts"
author:
...
We'll mostly be using the tidyverse to access the [**purrr**](https://purrr.tidyverse.org/) package. Note that we're also going to upgrade our data.table installation to the latest development version, which provides some new features that are not yet (as of the time of writing) available in the CRAN release. Let's install (if necessary) and load everything:
...
> **Tip:** If for some reason you have trouble installing the development version of data.table, see [here](https://github.com/Rdatatable/data.table/wiki/Installation). We'll also only be using a function or two from it today, so don't worry too much about upgrading if you are having difficulty.

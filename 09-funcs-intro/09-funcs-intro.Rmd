...
So you may not need to worry about explicit iteration at all. (Bonus: Vectorised operations are also extremely fast.) That being said, there are certainly cases where you will need to iterate. Let's explore with some toy examples --- some of which could be vectorised, but that's not the pedagogical point --- which provide a mental springboard for thinking about more complex cases.
...
*Note: Again, this is a rather silly function that we could easily improve upon using standard (vectorised) tools. But my goal here is to demonstrate programming principles with simple examples that carry over to more complicated cases.*
...
```{r multi_map2a, dependson=multi_func}

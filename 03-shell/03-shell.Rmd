...
... or, go through RStudio's build-in Terminal (likely, the most convenient way for most of you).
- Hitting `Shift`+`Alt`+`T` will cause a "Terminal" tab to open up in the bottom-left window pane (i.e. next to the "Console" tab).
- This should run Bash by default if it is installed on your system. Windows users: See [here](#wsl-rstudio).
...
I'm going to go out on a limb and recommend option 2 (WSL) if available to you. It's more overhead, but I think worth it. See the next few slides for instructions and tips...
...
- The WSL equivalent is "/mnt/c/Users/Grant/ec510/lectures".
- So, then you can navigate to today's lecture through the WSL with: `$ cd /mnt/c/Users/Grant/ec510/lectures/03-shell`. Adjust as needed.

**Tip:** If you use the WSL through RStudio's Terminal, then it will automatically configure the path (incl. mount point, etc.) to be the present working directing. Details on the next slide...

---
name: wsl-rstudio

# WSL + RStudio Terminal

While it's easy enough to open up your WSL (e.g. Ubuntu) shell on its own, I recommend that Windows users make WSL their default RStudio Terminal too. 

- In RStudio, navigate to: *Tools > Terminal > Terminal Options...*. \[[Screenshot](pics/wsl-rstudio-1.png).\]
- Click on the dropdown menu for *New terminals open with:*, select "Bash (Windows Subsystem for Linux)", and then click *OK*. \[[Screenshot](pics/wsl-rstudio-2.png).\]
...
As a Windows user, being able to access a true Bash shell conveniently from RStudio will make things much easier for you in my class. You can always revert back to a different shell later if you want. 
- I strongly recommend that you at least use Git Bash, though. More details [here](https://support.rstudio.com/hc/en-us/articles/115010737148-Using-the-RStudio-Terminal).

...
When we say that data.table "modifies by reference", that essentially means it modifies objects in place. This means lower memory overhead and faster computation time!
...
Here's a comparison with the dplyr equivalents from last week. I'll let you run and compare these yourself (note that the row order will change).
...
Quickly save the long-format stocks data.table. I'll use the `melt()` approach and also throw in some extra column-naming options, just so you can see those in action.
...
- If you find a great solution on Stackerflow that uses the "other" package... use it!
- Don't be a fanatic. Insisting on ecosystem purity is rarely worth it. 

.footnote[
<sup>1</sup> Remember my [admonition](https://raw.githack.com/uo-ec510-2020-spring/lectures/master/04-tidyverse/04-tidyverse.html#11) from last time: "A combination of tidyverse and base R is often the best solution to a problem." Add data.table to that list.

<span style='color: #FAFAFA;'><sup>2</sup>This does incur a performance penalty. But luckily there's a better solution on the next slide...</span>
]

--

</br>
Plus, as I hinted earlier, you *can* use tidyverse verbs on data.tables.<sup>2</sup> Try this yourself:
...
.footnote[
<sup>2</sup>This does [incur](https://stackoverflow.com/a/27513921) a performance penalty. But luckily there's a better solution on the next slide...
]
...
```{r dtplyr_call_ex}
lazy_dt(starwars) %>% filter(species=="humans") %>% group_by(gender) %>% summarise(height = mean(height, na.rm=T))
```
...
The tidyverse and data.table are often viewed as substitutes, but you can profitably combine both into your workflow... even if you strongly prefer one for (most of) your data wrangling needs.

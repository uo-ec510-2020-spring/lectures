...
- Like most forms of aggregation, `.N` is much more interesting when applied it is by group.
...
data.table's `by` argument functions very similarly to the `dplyr::group_by` equivalent. Try these yourself:
...
<span style='color: #FAFAFA;'><sup>2</sup> One annoyance I have is that the .() syntax doesn't work for .SDcols. However, note that you can feed it consecutive columns without quotes, e.g. .SDcols = height:mass. See here.</span>
]]
...
<sup>2</sup> One annoyance I have is that the `.()` syntax doesn't work for .SDcols. However, note that you can feed it consecutive columns without quotes, e.g. `.SDcols = height:mass`. See [here](https://rdatatable.gitlab.io/data.table/articles/datatable-intro.html#how-can-we-specify-just-the-columns-we-would-like-to-compute-the-mean-on).
]]

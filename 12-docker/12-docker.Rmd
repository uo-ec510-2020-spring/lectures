...
- Re-run your analyses after a major package update, only to find that the code now longer works or the results have changed?
...
[Docker](https://www.docker.com/) is by far the most widely used and best supported container technology.
- Certainly true for the types of problems that we are concerned with in this course.
- So, while there are container platforms around, when I talk about "containers" in this lecture, I'm really talking about Docker. 
...
Many deployment scenarios, but of particular interest to this course: When you start working with big data pipelines you want to be able to deploy software quickly and reliably. Need to run some code up on the cloud? Save time and installation headaches by running it through a suitable container (which can easily be deployed to a cluster of machines).

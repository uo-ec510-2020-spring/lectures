...
You should already have done this for the lecture on Google Compute Engine. See [here](https://raw.githack.com/uo-ec510-2020-spring/lectures/master/13-gce-i/13-gce-i.html) if not. 
...
```{r select_star_from, echo = FALSE, out.width='100%'}
...
if (fmt == "pdf_document"){
  message("Sorry, this image is only available in the the HTML version of the notes. See:\n",
...
- **Accessibility.** BigQuery is part of the Google Cloud Platform (GCP) that we signed up for in the [cloud computing lecture](https://raw.githack.com/uo-ec510-2020-spring/lectures/master/13-gce-i/13-gce-i.html). So you should already have *gratis* access to it. Sign up for a GCP [12-month free trial](https://console.cloud.google.com/freetrial) now if you haven't done so yet.
...
> **Tip:** The first line in the below code chunk allows us to auto-authenticate with BigQuery via the GCP service key that we downloaded in the [previous lecture](https://raw.githack.com/uo-ec510-2020-spring/lectures/master/14-gce-ii/14-gce-ii.html#google_cloud_api_service_account_key). (More on what's happening behind the scences [here](https://gargle.r-lib.org/articles/non-interactive-auth.html#provide-a-service-account-token-directly).) If you missed that lecture, or haven't created a service key yet, don't worry. You can still authenticate interactively in your browser. Just skip the first line (i.e. the `bq_auth()` bit) and run the second line. R will ask your whether it should cache credentials between sessions (I recommend "Yes") and then you'll be prompted to authenticate in your browser.

```{r bq_con_listtables}
bq_auth(path = Sys.getenv("GCE_AUTH_FILE")) ## Comment out if you'd rather authenticate manually
dbListTables(bq_con)
```
